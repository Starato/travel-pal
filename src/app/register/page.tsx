...
import VisibilityOff from "../../../public/visibility-off.png";

export default function page() {
  const [visible, setVisible] = useState<boolean>(false);
  const [form, setForm] = useState<string>('sign-in');

  const handleVisibility = (e: any): void => { 
    visible === false ? setVisible(true) : setVisible(false);
  }

  const handleFormChange = (): void => { 
    form === 'sign-in' ? setForm('sign-up') : setForm('sign-in')
  }
  ...
  return (
    <div className={styles["register-wrapper"]}>
      <div className={styles["register-form"]}>
        <div>
          <h1>{ form === 'sign-in' ? 'Sign In' : 'Sign Up' }</h1>
          <div>
            <input type="text" placeholder="Email"/>
          </div>
          <div>
            <Image width={24} height={24} onClick={ handleVisibility } src={ visible ? VisibilityOff : Visibility } alt="show password"/>
            <input type={ visible ? "text" : "password" } placeholder="Password"/>
          </div>
          <Link href="" className={ form === 'sign-up' ? styles["hidden-placehold"] : ""}>Forgot Password?</Link>
          <button value={ form }>
            { form === 'sign-in' ? 'Sign In' : 'Sign Up' }
          </button>
        </div>
        <div className={styles["form-change-wrapper"]}>
          <label>OR</label>
          <p onClick={ handleFormChange }>
            { 
              form === 'sign-in' 
              ? "Don't have an account? Sign up now!"
              : "Got an account already? Sign in now!"
            }
          </p>
        </div>
        <div className={styles["gsi-wrapper"]}>
